...
// magic numbers
const (
	MainnetMagic wire.BitcoinNet = 0xeafec591
...
func init() {
	// COLX mainnet Address encoding magics
...
// ColxParser handle
type ColxParser struct {
	*btc.BitcoinParser
	baseparser                         *bchain.BaseParser
	BitcoinOutputScriptToAddressesFunc btc.OutputScriptToAddressesFunc
}

// NewColxParser returns new ColxParser instance
func NewColxParser(params *chaincfg.Params, c *btc.Configuration) *ColxParser {
	p := &ColxParser{
...
func (p *ColxParser) ParseBlock(b []byte) (*bchain.Block, error) {
	r := bytes.NewReader(b)
	w := wire.MsgBlock{}
	h := wire.BlockHeader{}
	err := h.Deserialize(r)
	...
	if h.Version > 3 {
		// Skip past AccumulatorCheckpoint which was added in Colx block version 4
...
func (p *ColxParser) PackTx(tx *bchain.Tx, height uint32, blockTime int64) ([]byte, error) {
	return p.baseparser.PackTx(tx, height, blockTime)
}

// UnpackTx unpacks transaction from protobuf byte array
func (p *ColxParser) UnpackTx(buf []byte) (*bchain.Tx, uint32, error) {
	return p.baseparser.UnpackTx(buf)
}

// ParseTx parses byte array containing transaction and returns Tx struct
func (p *ColxParser) ParseTx(b []byte) (*bchain.Tx, error) {
	t := wire.MsgTx{}
	r := bytes.NewReader(b)
...
func (p *ColxParser) TxFromMsgTx(t *wire.MsgTx, parseAddresses bool) bchain.Tx {
...
func (p *ColxParser) ParseTxFromJson(msg json.RawMessage) (*bchain.Tx, error) {
	var tx bchain.Tx
...
func (p *ColxParser) outputScriptToAddresses(script []byte) ([]string, bool, error) {
...
func (p *ColxParser) GetAddrDescForUnknownInput(tx *bchain.Tx, input int) bchain.AddressDescriptor {
	if len(tx.Vin) > input {
