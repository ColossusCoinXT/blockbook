...
RUN \
  ...
  trap cleanup EXIT && \
  cd $GOPATH/src && \
  git clone https://github.com/ColossusCoinXT/blockbook.git && \
  cd blockbook && \
...
ADD ./build/blockchaincfg.json /build/blockchaincfg.json 

ADD ./build/blockbook  /build/blockbook

RUN chmod 777 /build/blockbook
